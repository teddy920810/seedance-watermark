...
function markdown(node) {
  ...
  if (node.nodeName === '#comment' || ['img', 'script', 'style'].includes(tag)) return '';
  const inner = () => children(node).map(markdown).join('').trim();
  ...
  if (['ul', 'ol'].includes(tag)) {
    return '\n\n' + children(node).filter((child) => child.tagName === 'li').map((child, index) => {
      ...
      return prefix + markdown(child).trim().replace(/\n/g, '\n' + ' '.repeat(prefix.length));
    }).join('\n') + '\n\n';
  }
  if (tag === 'table') {
    const rows = all(node).filter((child) => child.tagName === 'tr').map((row) =>
      children(row).filter((cell) => ['th', 'td'].includes(cell.tagName)).map((cell) => markdown(cell).trim().replace(/\|/g, '\\|').replace(/\n+/g, ' ')));
    if (!rows.length) return '';
    const lines = rows.map((row) => `| ${row.join(' | ')} |`);
    lines.splice(1, 0, `| ${rows[0].map(() => '---').join(' | ')} |`);
    return `\n\n${lines.join('\n')}\n\n`;
  }
  return children(node).map(markdown).join('');
}
...
export function importBlogHtml(html, publishedAt) {
  ...
  const body = [markdown(article), ...ctaParagraphs.map(markdown)].join('\n\n').replace(/\n[ \t]+\n/g, '\n\n').replace(/\n{3,}/g, '\n\n').trim();
  ...
  return { metadata, body, sourceSha256: createHash('sha256').update(html).digest('hex'), bodyText,
    markdown: `---\n${stringify(metadata)}---\n\n${body}\n` };
}
