...
- Node.js 22 LTS（项目支持 `>=22.19.0 <25`；安全修复后的图片依赖需要此下限）
- npm 10 或更高版本
- Git
...
每次开始需求时，先确认最终目标属于以下哪一种：

- **本地迭代**：只在本地实现和验证，不自动 commit、push、创建或合并 PR，也不部署。
- **合并 main**：完成验证、commit、push、PR、合并 `main`、等待部署，并按变更范围完成线上验证。
